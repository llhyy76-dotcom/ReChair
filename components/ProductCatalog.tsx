...
type Product = {
  id:string; brand:string; model_name:string; title:string; price:number; grade:string; status:string;
  year_text?:string|null; region?:string|null; thumbnail_url?:string|null; is_featured?:boolean;
};
...
export default function ProductCatalog(){
  ...
  const filtered=useMemo(()=>{
    ...
    const arr=products.filter(p=>
      ...
      (!brand||p.brand===brand)&&(!grade||p.grade===grade)
    );
    return [...arr].sort((a,b)=>{
  ...
  return <section className="rc-store-section"><div className="rc-store-container">
    ...
    {loading?<div className="rc-store-empty">상품을 불러오는 중입니다.</div>:filtered.length===0?<div className="rc-store-empty">조건에 맞는 상품이 없습니다.</div>:
    <div className="rc-store-grid">{filtered.map(p=><a className="rc-store-card" href={`/products/${p.id}`} key={p.id}>
      <div className="rc-store-image">{p.thumbnail_url?<img src={p.thumbnail_url} alt={p.title}/>:<span>💺</span>}{p.is_featured&&<i>추천</i>}<b>{p.grade}</b></div>
      ...
      <strong>{money(p.price)}</strong><em>상품 상세보기 ›</em></div>
    </a>)}</div>}
  </div></section>;
}
