...
export default function AdminScheduleCalendar(){
 ...
 <section className="ops-board">{groups.map(g=><article key={g.name}><div className="tech-head"><h2>{g.name}</h2><span>{g.items.length}건</span></div><div className="tech-list">{g.items.length?g.items.map(i=><button key={i.id} onClick={()=>setSelected(i)}><time>{new Date(i.scheduled_at).toLocaleTimeString('ko-KR',{hour:'2-digit',minute:'2-digit'})}</time><div><b>{i.customer_name}</b><span>{i.region||'지역 미입력'} · {i.service_type||'서비스 미입력'}</span><em>{i.address||'주소 미입력'}</em></div><small>{i.status}</small></button>):<p>일정 없음</p>}</div></article>)}</section>
