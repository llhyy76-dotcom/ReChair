...
export async function PATCH(request:NextRequest, context:Ctx) {
  try {
    const {id}=await context.params; const body=await request.json(); const supabase=getSupabaseServer();
    ...
    return NextResponse.json({error:error instanceof Error?error.message:'상품 수정 오류'},{status:500});
  }
}

export async function DELETE(_:NextRequest, context:Ctx) {
  try {
    const {id}=await context.params; const supabase=getSupabaseServer();
